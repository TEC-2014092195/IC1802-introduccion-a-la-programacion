    ...
    def AccionF1(btn,btn2):
        ...
        def validarcallback(elegido,cambiante1,cambiante2,orig1,orig2):
            ...
            if cambiante1 == orig1 and cambiante2 != elegido:
                ...
                messagebox.showinfo("BM","Ya esta ocupado, deseleccione alguno para cambiar de valor")
    ...
    def AccionF2(btn2,btn3):
        ...
        def callback1(event):
            if validarfase() == True: #Puede continuar eligiendo
                ...
                messagebox.showinfo("BM","Debe agregar todos los ganadores de la Fase\nHasta que todos esten listos podra elegir la otra Fase")
...
##################################################################################################################
    def AccionFGanador8(btn,btn2,btn3,btn4,btn5,btn6,id_bracket,v_ganador,v_main):
...
##                print("Completo")
        ...
        def callback1(event):
            ...
            if flag == False:
                ...
                file=open(("Usuarios/Registrados/user_"+id_bracket+".txt"), 'w')
...
#-----------------Finalizando Guardado
                file=open("Usuarios/Usuarios.txt","r+")
                lista=file.readlines()
                m=""
                for i in range(len(lista)):
                    if lista[i].split(",")[0]==id_bracket:
                        m=lista[i].replace("Nuevo","Registrado")
                        lista[i]=m   #m almacena la linea cambiada para despues modificar la lista
...
##################################################################################################################
##################################################################################################################
    def AccionFGanador4(btn,btn2,btn3,btn4,btn5,id_bracket,v_ganador,v_main):
...
##################################################################################################################
##################################################################################################################
    def AccionFGanador2(btn,btn2,btn3,btn4,id_bracket,v_ganador,v_main):
